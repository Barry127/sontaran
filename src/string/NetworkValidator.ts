...
export class NetworkValidator extends StringValidator {
  ...
  ip() {
    this.validators.push((value: string) =>
      value.includes(':') ? ipv6RegExp.test(value) : ipv4(value)
    );
    return this;
  }
  ...
  ipv4() {
    this.validators.push(ipv4);
    return this;
  }

  /** Expect value to be a valid IPv6 address */
  ipv6() {
    return this.match(ipv6RegExp);
  }

  /** Expect value to be a valid MAC address */
  mac() {
    return this.match(/^([0-9a-f]{2}:){5}[0-9a-f]{2}$/i);
  }
...
export const network = () => new NetworkValidator();
...
const ipv4 = (value: string): boolean => {
  ...
  return parts.reduce((valid, part) => {
    ...
    if (numericValue < 0 || numericValue > 255) return false;
